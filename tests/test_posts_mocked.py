...
def test_get_post_mocked(mock_get):
    """Mock the HTTP call — no real network required."""
...
def test_network_failure_raises(mock_get):
    """Simulate a network failure using side_effect."""
...
def test_create_post_mocked(mock_post):
    """Mock a POST request — verify payload is accepted and ID returned."""
    mock_response = MagicMock()
...
def test_get_post_with_mocker(mocker, api_client):
    """Same mock as above — pytest-mock syntax for comparison."""
